...
/*
 * ADC is configured to read temp and AIN0...AIN12 = total of 13 channels.
 * Reading initialized in ec_MAX1231_init() and re-inited in EOC interrupt
 * Results are stored in cfg_table->data (also all_adc_readings[])
 */

volatile ec_MAX1231_cfg_t * cfg_t_ptr[3];
...
void ec_MAX1231_init(ec_MAX1231_cfg_t *cfg_table)
{
    ...
    SPI_master_swapData(SPI_PORT_ADC, (uint8_t) ((0b01 << 6) | (CKSEL_bits << 4) | (REFSEL_bits << 2) | (DIFFSEL_bits)));
    // Write to unipolar / bipolar config register
    SPI_master_swapData(SPI_PORT_ADC, (uint8_t) (0b00000000)); // AIN0...AIN15 are set to unipolar mode
    ...
    SPI_master_swapData(SPI_PORT_ADC, (uint8_t) ((0b01 << 6) | (CKSEL_bits << 4) | (REFSEL_bits << 2) | (DIFFSEL_bits)));
    SPI_master_swapData(SPI_PORT_ADC, (uint8_t) (0b00000000)); // none of AIN0...AIN15 are set to bipolar mode
    ...
    SPI_master_swapData(SPI_PORT_ADC, (uint8_t) ((1 << 7) | (CHSEL_bits << 3) | (SCAN_bits << 1) | (TEMP_bits)));
    ...
    SPI_master_swapData(SPI_PORT_ADC, (uint8_t) ( (0b001 << 5) | (AVGON_bits << 4) | (NAVG_bits << 2) | (NSCAN_bits) ));*/
    ...
    if (DMA_already_inited == 0)
    {
        ...
        DMA0CTL |= DMADT_0 + DMADSTINCR_2 + DMASRCBYTE_L + DMADSTBYTE_L;
        ...
        DMA1CTL |= DMADT_0 + DMASRCBYTE_L + DMADSTBYTE_L;
...
{
    if ( (cfg_t_ptr[0]->ready == ADC_ready)
            && (cfg_t_ptr[1]->ready == ADC_ready)
...
void ec_MAX1231_reset(ec_MAX1231_cfg_t *cfg_table)
{
    ...
    // Write reset to ADC
    SPI_master_swapData(SPI_PORT_ADC, 0b00010000);
    // Write reset FIFO to ADC
    SPI_master_swapData(SPI_PORT_ADC, 0b00011000);
}
...
void __attribute__ ((interrupt (PORT2_VECTOR))) Port_2(void)  //adc0 p2.3
{
    if (P2IFG & BIT3)
    {
        current_ADC_index = 0;
        ...
        DMA1SZ = (cfg_t_ptr[0]->N_channels) * 2;
        ...
        DMA0CTL |= DMAEN | DMAIE;
        DMA1CTL |= DMAEN;
        SPI_master_swapData(SPI_PORT_ADC, 0);
    }
}
...
void __attribute__ ((interrupt (PORT3_VECTOR))) Port_3(void)
{
    if (P3IFG & BIT7)
    {
        ...
        DMA1SZ = (cfg_t_ptr[1]->N_channels) * 2;
    ...
    else if (P3IFG & BIT5)
    {
        ...
        DMA1SZ = (cfg_t_ptr[2]->N_channels) * 2;
